...
let
  ...
  dirAttrs =
    ...
    mapAttrs' (name: _: fileAttrInPath suffix path name) (
      filterAttrs (name: type: hasSuffix suffix name && type == "regular") (builtins.readDir path)
    );
  ...
  evolve =
    ...
    lib.mapAttrs (
      k: v:
      if lib.hasAttr k funs then
        let
          mapper = funs."${k}";
        in
        if lib.isAttrs mapper then evolve mapper v else mapper v
      else
        v
    ) vals;
  ...
  importRest =
    { pkgs, ... }@args:
    path:
    (mapAttrs (_: file: pkgs.callPackage file args) # import with the same args, add `...` arg if needed
          ...
          if strings.typeOf dep == "string" then
            {
              ...
              value =
                ...
                in
                if packages ? dep then packages."${dep}" else packages.default;
            }
          else
            mapAttrs (
              k: v: {
      ...
      // {
        "${k}" =
          let
            v = getAttr k lower;
          in
          if o ? k then (getAttr k o) ++ v else v;
      }
  ...
  invert =
    ...
    foldl' (
      ...
      acc
      // {
        "${value}" = (if hasAttr value acc then acc."${value}" else [ ]) ++ [ name ];
      }
    ) { } (attrsToList obj);
  ...
  scripts = invert (
    listToAttrs (
      lists.map (
        fname:
        let
          frag = elemAt (splitString "." fname);
        in
        nameValuePair (frag 0) (frag 1)
